...
class AddNonDefaultAddressController extends GetxController {
  Either<NetworkException, String>? result;

  Future<void> addingAddress(BuildContext context, AddressAddingParams addressAddingParams) async {
    showDialog(
      context: context,
    ...
    result = await AddressRepository().addNonDefaultAddress(addressAddingParams);
    Get.back();
    result?.fold(
          (l) {
